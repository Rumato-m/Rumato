document.addEventListener('DOMContentLoaded', () => {
  ...
  const SOFA_BASE = 'images/sofas/Nonoro';
  ...
  const PLACEHOLDER_IMG = (() => {
    const svg = encodeURIComponent(`<svg xmlns='http://www.w3.org/2000/svg' width='1200' height='400'>
      ...
      <text x='50%' y='50%' dominant-baseline='middle' text-anchor='middle'
        ...
        Попередній перегляд дивана Nonoro
      </text>
    </svg>`);
  ...
  const state = {
    ...
    // Новые селекты (полоса под героем)
    sleepplace: '160x200',  // спальное место; берём первые три цифры
    lift: false,            // подъёмный механизм (в имя файла НЕ входит)
    border: 20              // толщина борта: 20 | 10 | 5  → в файле "20" | "10" | "05"
  };
  ...
  const el = {
    ...
    len2: $('len2Select'),
    shape: $('shapeSelect'),
    preview: $('previewImg'), // может отсутствовать (старый одиночный превью)
    price: $('priceValue'),
  ...
  // =========================
  // Превью (левое/правое) + поддержка старого одиночного
  // =========================
  function updatePreview(){
    const urlLeft  = buildFilename();
    const urlRight = buildFilenameTop();

    // старое одиночное превью, если ещё присутствует в вёрстке
    if (el && el.preview){
    ...
    // ПРАВОЕ окно (вид сверху с суффиксом V)
    if (previewRight){
  ...
  function resolveValidURL(basePath, name){
    ...
    return new Promise((resolve) => {
      let settled = false;
      const onOK = (u) => { if (!settled){ settled = true; resolve(u); } };
  ...
  // =========================
  // Обработчики исходного конфигуратора (если элементы присутствуют)
  // =========================
  if (el.shape){
  ...
  // =========================
  // Новые селекты в полосе фильтров
  // =========================
  const sleepSizeEl = $('sleepSize');
  const borderEl    = $('borderSelect');
  const liftEl      = $('liftSelect');

  sleepSizeEl?.addEventListener('change', (e)=>{
    state.sleepplace = e.target.value;
    updatePreview();
  });

  borderEl?.addEventListener('change', (e)=>{
    state.border = Number(e.target.value);
    updatePreview();
  });

  liftEl?.addEventListener('change', (e)=>{
  ...
  document.querySelectorAll('.addon-ctrl').forEach(block => {
    ...
    let qty = 1;
    function sync(){ if (qtyVal) qtyVal.textContent = qty; }
    sync();
  ...
  const articleContent = {
    ...
    color: { title: 'Колір дивана як мова інтерʼєру', body: '...' },
  ...
  // Небольшая самопроверка вычислений (не влияет на UI)
  (function tests(){
    try{
      const bak = {...state};
      state.shape='straight'; state.len1=160; state.depth=80; state.chaise='none'; state.sleep=false; state.fabric=null; state.lift=false; state.border=20;
      console.assert(computePrice()===24000, '160 см → 24 000');
      state.len1=260; console.assert(computePrice()===25600, '260 см → 25 600');
      state.depth=100; console.assert(computePrice()===27600, '260 см + глубина → 27 600');
      state.fabric='f02'; console.assert(computePrice()===30360, 'с тканью f02 (×1.1)');
      const p0 = computePrice();
      state.lift=true; state.border=10;
      console.assert(computePrice() > p0, 'lift + border10 увеличивают цену');
